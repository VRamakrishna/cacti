...
if [ "$MODE" = "up" ]; then
    ...
    #for ii in $(seq 1 $NUM_NW); do
        for org in $(seq 1 $NUM_ORGS); do
            ...
            . docker-testnet/${env_dir}/.env.n${NW_ID}.org${org}
            chmod 777 ${DLT_SPECIFIC_DIR}
            #docker compose --env-file docker-testnet/${env_dir}/.env.n${ii}.org${org} up -d
            docker compose --env-file docker-testnet/${env_dir}/.env.n${NW_ID}.org${org} up -d
        done
    #done
else
    #for ii in $(seq 1 $NUM_NW); do
        for org in $(seq 1 $NUM_ORGS); do
            ...
            docker compose --env-file docker-testnet/${env_dir}/.env.n${NW_ID}.org${org} down
        done
    # done
