...
docker logs driver-fabric-network2 | grep -a VIEW_GENERATION | tail -n 100 > $OUTDIR/latency.conf_ecies.vg.$1.$3
for j in $(docker ps | grep interop | awk '{print $1}'); do docker logs $j | grep HANDLE_EXTERNAL_REQUEST | tail -n 100; done > $OUTDIR/latency.conf_ecies.her.$1.$3
for j in $(docker ps | grep interop | awk '{print $1}'); do docker logs $j | grep WRITE_EXTERNAL_STATE | tail -n 100; done > $OUTDIR/latency.conf_ecies.wes.$1.$3
